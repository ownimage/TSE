...
public class TransformResultBatch implements ITransformResultBatch {
	...
	public boolean hasNext() {
		...
		if (mXCurrent < 0) { throw new IllegalStateException(String.format("mXCurrent (%d) must be greater than 0.", mXCurrent)); }
		if (mYCurrent < 0) { throw new IllegalStateException(String.format("mYCurrent (%d) must be greater than 0.", mYCurrent)); }
		...
		if (!hasNext) {
			...
			System.out.println(String.format("Total batch, batchSise=%d, millisecs=%d", mPixelsProcessed, (end - start)));
		}
