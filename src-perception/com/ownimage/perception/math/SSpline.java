...
public class SSpline {
	...
	public double evaluate(final double pX) {
		if (pX < mPointA.getX()) { return evaluateLower(pX); }
		if (pX < mPointB.getX()) { return evaluateMiddle(pX); }
	...
	double evaluateMixed(final double pX, final Point pA) {
		...
		if (C >= gradient) { return cubic.evaluate(pX); }
