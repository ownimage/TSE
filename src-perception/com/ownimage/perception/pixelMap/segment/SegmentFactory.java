...
public class SegmentFactory {
	...
	/**
	 * Changes the supplied pSegment to a CurveSegment. If the pSegment is already a CurvetSegment this will do nothing and return
	 * pSegment. Otherwise it will create a new CurveSegment from the start and end vertexes of pSegment and attach it to those
	 * vertexes. t is intended to be called from user action changing the types of segments.
	 * 
	 * @param pSegment
	...
	static public ISegment changeToCurveSegment(final ISegment pSegment) {
		...
		if (pSegment instanceof CurveSegment) { return pSegment; }
	...
	/**
	 * Changes the supplied pSegment to a DoubleCurveSegment. If the pSegment is already a DoubleCurveSegment this will do nothing
	 * and return pSegment. Otherwise it will create a new DoubleCurveSegment from the start and end vertexes of pSegment and attach
	 * it to those vertexes. It is intended to be called from user action changing the types of segments.
	 * 
	 * @param pSegment
	 *            the segment. This must not be null.
	 * @return the Segment that joins the start and end vertexes.
	...
	static public ISegment changeToDoubleCurveSegment(final ISegment pSegment) {
		...
		if (pSegment instanceof DoubleCurveSegment) { return pSegment; }
	...
	/**
	 * Changes the supplied pSegment to a StraightSegment. If the pSegment is already a StraightSegment this will do nothing and
	 * return pSegment. Otherwise it will create a new StrightSegment from the start and end vertexes of pSegment and attach it to
	 * those vertexes. It is intended to be called from user action changing the types of segments.
	 * 
	 * @param pSegment
	 *            the segment. This must not be null.
	...
	static public ISegment changeToStraightSegment(final ISegment pSegment) {
		...
		if (pSegment instanceof StraightSegment) { return pSegment; }
	...
	/**
	 * Creates the curve segment starting at pStart, ending at pEnd and going through pThrough. In the event that the line collapses
	 * to a straight line then it will return a LineApproximation instead.
	 * 
	 * @param pStart
	 *            the start vertex
	 * @param pEnd
	...
	/**
	 * Creates the curve segment starting at pStart, ending at pEnd and the gradient of the curve at the start and end is towards
	 * pP1. In the event that the line collapses to a straight line then it will return a LineApproximation instead.
	 * 
	 * @param pStart
	 *            the start vertex
	 * @param pEnd
	...
	public static ISegment createTempDoubleCurveSegment(final IVertex pStartVertex, final Line pStartTangent, final IVertex pEndVertex, final Line pEndTangent, final IVertex pThrough) {
		...
		if (startP1 == null || endP1 == null) { return SegmentFactory.createTempStraightSegment(pStartVertex, pEndVertex); }
