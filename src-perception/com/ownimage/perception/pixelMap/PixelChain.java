...
import com.ownimage.perception.pixelMap.editor.EPMDGraphicsHelper;
...
public class PixelChain implements Serializable {
	...
	public PixelChain(final Node pStartNode) {
		if (pStartNode == null) { throw new IllegalArgumentException("pStartNode must not be null"); }
	...
	/**
	 * Adds the two pixel chains together. It allocates all of the pixels from the pOtherChain to this, unattaches both chains from
	 * the middle node, and adds all of the segments from the second chain to the first (joining at the appropriate vertex in the
	 * middle, and using the correct offset for the new vertexes). Note that the new segments that are copied from the pOtherChain
	 * are all LineApproximations.
	 * 
	 * @param pOtherChain
	 *            the other chain
	 */
	private void add(final PixelChain pOtherChain) {
		...
		if (getPixelMap() != pOtherChain.getPixelMap()) { throw new IllegalArgumentException("PixelMap must be the same for both chains"); }

		if (!lastPixel()
				.equals(pOtherChain.firstPixel())) { throw new IllegalArgumentException("PixelChains not compatible, last pixel of this:" + this + " must be first pixel of other: " + pOtherChain); }

		final int offset = mPixels.size() - 1; // this needs to be before the removeElementAt and addAll. The -1 is because the end
	...
	public void addSegment(final ISegment pSegment) {
		...
		if (pSegment == null) { throw new IllegalArgumentException("pSegment must not be null"); }

		if (mSegments.size() == 0 && pSegment.getStartIndex() != 0) { throw new IllegalArgumentException("First segment that is added must have a startIndex of 0"); }
	...
	public void approximate01_straightLines(final double pTolerance) {
		...
		// there are cases where a line of length n will be close enough, a line of length n+1 will not be, but there exists an m
		// such that a line of length m is close enough.
		mSegments = new Vector<ISegment>();

		if (mPixels.size() <= 1) { return; }
	...
	public void approximate02_refineCorners() {
		...
		for (final ISegment currentSegment : mSegments) {
			if (currentSegment == mSegments.lastElement()) { return; }
			...
			if (nextSegment == null) { throw new IllegalStateException("Only the final segment in a PixelChain can have no EndVertex's EndSegment"); }
			...
			// TODO the check below is needed as some segments may only be one index length so generating a midpoint might generate
			// an invalid segment
	...
	private IVertex createVertex(final int pIndex) {
		if (pIndex < 0
				|| pIndex >= mPixels.size()) { throw new IllegalArgumentException("pIndex =(" + pIndex + ") must lie between 0 and the size of the mPixels collection =(" + mPixels.size() + ")"); }
	...
	public void deleteVertex(final IVertex pVertex) {
		...
		if (pVertex.getPixelChain() != this) { throw new IllegalArgumentException("pVertex belongs to PixelChain: " + pVertex.getPixelChain() + ".a\n This is PixelChain: " + this); }
		if (!checkVertexIsInChain(pVertex)) { throw new IllegalArgumentException("pVertex: " + pVertex + " is not properly connected in this PixelChain: " + this); }
	...
	/**
	 * Gets fraction of the way along the end segment that the pPosition is. 0 would mean at the thinnest end. 1 would mean full
	 * thickness.
	 * 
	 * @param pPosition
	 *            the position
	...
	private Line getDCEndTangent(final ISegment pSegment) {
		if (pSegment == null) { throw new IllegalArgumentException("Segment must not be null."); }

		final IVertex endVertex = pSegment.getEndVertex();
		if (endVertex.getEndSegment() instanceof StraightSegment) { return endVertex.getEndSegment().getStartTangent(); }
	...
	private Line getDCStartTangent(final ISegment pSegment) {
		if (pSegment == null) { throw new IllegalArgumentException("Segment must not be null."); }

		final IVertex startVertex = pSegment.getStartVertex();
		if (startVertex.getStartSegment() instanceof StraightSegment) { return startVertex.getStartSegment().getEndTangent(); }
	...
	public IVertex getJoinVertex() {
		if (mSegments.size() == 0) { return getStartVertex(); }
	...
	public Pixel getPixel(final int pIndex) {
		if (pIndex < 0 || pIndex > length()) { throw new IllegalArgumentException("pIndex, currently: " + pIndex + " must be between 0 and the length of mPixels, currently: " + length()); }
	...
	public IVertex getStartVertex() {
		if (mSegments == null || mSegments.size() == 0) { return mStartVertex; }
	...
	// TODO can remove the pSelectedVertex from this as this is drawn separately .. The reason for this is that the selected Vertex
	// might be behind a non=selected one.
	...
	private boolean isValid(final ISegment pSegment) { // need to maks sure that not only the pixels are close to the line but the
														// line is close to the pixels
	...
	/**
	 * Merges two pixel chains together that share a common Node. The result is one PixelChain with a vertex where the Node was. The
	 * chain will have correctly attached itself to the node at either end. This needs to be done before after the segments are
	 * generated so that the vertex for the node can be created.
	 * 
	 * @param pOtherChain
	 *            the other chain
	 * @param pNode
	...
	public void merge(final PixelChain pOtherChain, final Node pNode) {
		if (!(getStartNode() == pNode || getEndNode() == pNode) || !(pOtherChain.getStartNode() == pNode || pOtherChain.getEndNode() == pNode)) { throw new IllegalArgumentException(
				"Either this PixelChain: " + this + ", and pOtherChain: " + pOtherChain + ", must share the following node:" + pNode); }
		...
		if (getEndNode() != pNode || pOtherChain
				.getStartNode() != pNode) { throw new RuntimeException("This PixelChain: " + this + " should end on the same node as the other PixelChain: " + pOtherChain + " starts with."); }
			...
			try {
				...
				if (currentSegment == mSegments.firstElement()) {
					...
					try {
						...
						for (int i = 1; i < currentSegment.getPixelLength(); i++) { // first and last pixel will throw an error and
																					// are equivalent to the straight line
							try {
					...
					try {
						...
						try {
							...
							if (startTangent != null && endTangent != null) {
								...
								if (startTangent.closestLambda(p1) > 0.0d && endTangent.closestLambda(p1) < 0.0d) {
									...
									// if (p1 != null && newStartTangent.getAB().dot(startTangent.getAB()) > 0.0d &&
									// newEndTangent.getAB().dot(endTangent.getAB()) > 0.0d) {
						...
						// try {
						// for (int i = startVertex.getIndex() + 2; i < endVertex.getIndex() - 2; i++) { // first and last pixel
						// will throw an error and are equivalent to the straight line
						// try {
						// final Point through = getPixel(i).getUHVWPoint();
						// final ISegment candidateCurve = SegmentFactory.createTempCurveSegmentThrough(startVertex, endVertex,
						// through);
						//
						// final double candidateError = candidateCurve.calcError();
						//
	...
	private void refine03_matchDoubleCurves() {
		...
		for (final ISegment currentSegment : mSegments) {
			...
			try {
				try {
					for (int i = 1; i < currentSegment.getPixelLength() - 1; i++) { // first and last pixel will throw an error and
																					// are equivalent to the straight line
					...
					if (bestSegment instanceof DoubleCurveSegment) {
						// final LineSegment lineSegment = new LineSegment(currentSegment.getStartUHVWPoint(),
						// currentSegment.getEndUHVWPoint());
						// currentSegment.getEndVertex().setTangent(lineSegment);
	...
	/**
	 * Reverses the order of the pixels in the pixel chain. This means reversing the start and end nodes. And all of the segments in
	 * the line are also reversed and repaced with new straight line segments.
	 */
	...
	public void setPixelMap(final PixelMap pPixelMap) {
		...
		if (mPixelMap != null) { throw new IllegalStateException("Trying to overwrite a non null mPixelMap value"); }
	...
	public void setStartNode(final Node pNode) {
		...
		if (mStartNode != null) { throw new IllegalStateException("Can not change the start node of a PixelChain once it has been set.  Current mStartNode = " + mStartNode); }
		if (pNode == null) { throw new IllegalArgumentException("Can not change the start node to a null value"); }
