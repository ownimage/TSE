...
public class ImageOrganizer extends AppControlBase {
    ...
    protected IView createContentView() {
        ...
        mMasterDirectory.addControlChangeListener(mMasterDirectoryChangeListener = this::masterDirectoryChangeListener);
        mSplit = new ActionControl("Split", "split", container, this::split).setEnabled(false);
        ...
        mReorgJPG = new ActionControl("Reorg JPG", "reorgJPG", container, this::reorgJPG).setEnabled(false);
        mReorgNEFDir = new StringControl("NEF Dir", "nefDir", container, "").setEnabled(false);
        mReorgNEF = new ActionControl("Reorg NEF", "reorgNEF", container, this::reorgNEF).setEnabled(false);
    ...
    protected MenuControl createMenuView() {
        ...
        MenuControl menu = new MenuControl.Builder()
                .addMenu(new MenuControl.Builder().setDisplayName("File")
                        .addAction(new ActionControl("Exit", "exit", menuContainer, this::fileExit))
                        .build())
                .build();
    ...
    public void masterDirectoryChangeListener(final Object pSource, final boolean pIsMutating) {
        if (pSource != mMasterDirectory) {
            throw new IllegalStateException("wrong object specified");
        }
    ...
    private File mkdir(final File pParentDir, final String pDirName) throws Exception {
        ...
        } else if (!dir.isDirectory() && !dir.mkdir()) {
            throw new Exception("Unable to create directory " + pDirName + " in " + pParentDir.getAbsolutePath());
        }
        return dir;
    }
    ...
    private void renameTo(final File pToDir, final File pFile) throws Exception {
        if (pFile.isFile() && !pFile.renameTo(new File(pToDir, pFile.getName()))) {
            throw new Exception("Unable to move file: " + pFile.getAbsolutePath());
        }
    ...
    private void renameTo(final File pFromDir, final File pToDir, final String pFromName, final String pToName) throws Exception {
        ...
        if (f.isFile() && !f.renameTo(new File(pToDir, pToName))) {
            throw new Exception("Unable to move file: " + f.getAbsolutePath() + " to " + pToDir.getAbsolutePath() + File.separator + pToName);
        }
    }
    ...
    private void reorgNEF() {
        ...
        try {
            reorgNef(mReorgNEFDir.getValue());
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private void split() {
        ...
        try {
            splitJpgNef(mMasterDirectory.getValue());
        } catch (Exception e) {
            e.printStackTrace();
        }
     ...
     *
     * @param pDirectoryName the name of the top level directory
     * @throws Exception
     */
