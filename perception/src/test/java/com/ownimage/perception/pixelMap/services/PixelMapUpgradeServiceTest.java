    ...
    public void upgradeSegments() {
        ...
        IntStream.range(4, 9).boxed()
                .map(i -> {
                            ...
                            if ((i & 1) == 0) {
                                var curveSegment = mock(CurveSegment.class);
                                when(curveSegment.getSegmentIndex()).thenReturn(index);
                                when(curveSegment.getStartPosition()).thenReturn(position);
                                when(curveSegment.getA()).thenReturn(a);
                                when(curveSegment.getB()).thenReturn(b);
                                when(curveSegment.getP1()).thenReturn(p1);
                                return curveSegment;
                            }
                            return mock(StraightSegment.class);
                        }
                )
        ...
        for (int i = 0; i < actual.getSegments().size(); i++) {
            if (ImmutableCurveSegment.class != actual.getSegment(i).getClass()) {
                assertSame(pixelChain.get().getSegment(i), actual.getSegment(i));
            } else {
                assertSame(ImmutableCurveSegment.class, actual.getSegment(i).getClass());
                var expected = ImmutableCurveSegment.copyOf((CurveSegment) pixelChain.get().getSegment(i));
                assertTrue(actual.getSegment(i).equals(expected));
                assertEquals(expected, actual.getSegment(i));
            }
        }
    }
