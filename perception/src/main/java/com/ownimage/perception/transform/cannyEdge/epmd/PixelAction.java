...
public interface PixelAction extends Serializable {
    ...
    ImmutablePixelAction defaultPixelAction = ImmutablePixelAction.of("NoName", -1,
            defaultClickEvent, defaultControlVisibility, defaultDragEvent, defaultDragEndEvent);

    static ImmutablePixelAction of(String name, int cursorSize, BiFunction<IUIEvent, Pixel, Boolean> clickEvent) {
        return defaultPixelAction.withName(name).withCursorSize(cursorSize).withClickEvent(clickEvent);
    }
