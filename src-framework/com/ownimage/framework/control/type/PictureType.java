...
public class PictureType implements IType<NullMetaType<PictureType>, PictureType>, IPictureSource {
	 ...
	 *
	 * @param pFile
	 *            the file that holds the image data.
	 * @throws FrameworkException
	 *             if there is any underlying problem in instantiating the image.
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.ownimage.perception.control.type.IType#duplicate()
	 */
	@Override
	...
	public synchronized Color getColor(final int pX, final int pY) {
		...
		if (!is360() && (pX < 0 || pX >= getWidth())) { return mColorProperty.getValue(); }
	...
	public Color getColor(final IntegerPoint pPoint) {
		if (pPoint == null) { throw new IllegalArgumentException("pIn must not be null"); }
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.ownimage.perception.control.type.IType#getMetaModel()
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.ownimage.perception.control.type.IType#getString()
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.ownimage.perception.control.type.IType#getValue()
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.ownimage.perception.util.IPictureIntArray#getColors(int[])
	 */
	...
	public void save(final File pFile) throws Exception {
		...
		if (!("jpg".equals(extension) || "gif".equals(extension))) { throw new IllegalArgumentException("Extension of file " + pFile.getAbsolutePath() + " + is ." + extension
	...
	public synchronized void setColor(final int pX, final int pY, final Color pColor) {
		...
		if (pColor == null) { throw new IllegalArgumentException("can't set Color to null"); }
		if (pY < 0 || pY >= getHeight()) { throw new IllegalArgumentException(
				"pY = " + pY + ", it needs to be between 0 and getHeight()-1 [" + (getHeight() - 1) + "]"); }

		if (!is360() && (pX < 0 || pX >= getWidth())) { throw new IllegalArgumentException(
	...
	public void setColor(final IntegerPoint pPoint, final Color pColor) {
		...
		if (pPoint == null) { throw new IllegalArgumentException("pPoint must not be null"); }

		if (pColor == null) { throw new IllegalArgumentException("pColor must not be null"); }
	...
	public void setColors(final int[] pColors) {
		...
		if (pColors == null) { throw new IllegalArgumentException("pColors mst not be null"); }
		if (pColors.length != getHeight() * getWidth()) { throw new IllegalArgumentException(
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.ownimage.perception.control.type.IType#setString(java.lang.String)
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.ownimage.perception.control.type.IType#setValue(java.lang.Object)
	 */
	@Override
	...
	private void testIsLocked() {
		if (isLocked()) { throw new IllegalStateException("Cannot modify PictureType when it is locked."); }
	}
	...
	public void unlock(final int pKey) {
		...
		if (!isLocked()) { throw new IllegalStateException("The PictureType is not locked"); }
		if (mLock != pKey) { throw new IllegalArgumentException("Wrong key given to unlock PictureType."); }
