...
public class UndoRedoBuffer implements IUndoRedoBuffer, IUndoRedoBufferProvider {
	...
	public void endSavepoint(final Id pId) {
		...
		if (mSavePoint == null) { throw new IllegalArgumentException("No savepoint active"); }
		...
		if (mSavePoint.getId() == pId) {
			...
			if (!mSavePointIds.remove(pId)) { throw new IllegalArgumentException("savepoint not in currenlty active set"); }
		}
	}
	...
	public synchronized boolean redo() {
		...
		if (mSavePoint != null) { throw new IllegalStateException("Cannot call redo when a savepoint is active."); }

		boolean redo = false;

		if (mPointer < mStack.size()) {
			mStack.get(mPointer).redo();
			mPointer++;
			redo = true;
		}
