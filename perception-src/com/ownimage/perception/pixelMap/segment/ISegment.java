...
public interface ISegment extends ITestableLine, Serializable {
	...
	/**
	 * Attach to vertexes. A number of segments might be defines as being between the same two vertexes, this is done to see which
	 * one of the segments most closely approximates the underlying pixels. When one is finally chosen as the best this segment
	 * needs to be attached to the its vertexes, or more precisely the vertexes need to be updated to they attach to this segment.
	 */
	...
	/**
	 * This returns the Point that is the mean of all of the Points that correspond to the Pixels in the Pixel chain that are
	 * approximated by this Segment.
	 * 
	 ...
	 * 
	 * @return the end tangent. This is a vector that points along the tangent to a point beyond the end, i.e. towards a point that
	 *         it would join with.
	 */
	...
	/**
	 * Pixel length is defined as the number of pixels represented by this Segment. It is defined as: if the startIndex is 0 then (1
	 * + endIndex), else (endIndex - startIndex). This definition means that if you add all the pixelLenghs of the segments that
	 * approximate a PixelChain then you get the length of the chain.
	 * 
	...
	/**
	 * Gets the start tangent. This is a vector that points along the tangent to a point before the start, i.e. towards a point that
	 * it would join with.
	 * 
	...
	public void graffiti(EPMDGraphicsHelper pGraphics);
	...
	/**
	 * Checks each pixel in the original PixelChain to see if it is further than pDistance away. Note that all the checks are done
	 * in the UHVW format. The check starts at the mStart end and works forwards.
	 * 
	 * @param pDistance
	 *            the distance
	...
	/**
	 * Checks each pixel in the original PixelChain to see if it is further than mDistance away. Note that all the checks are done
	 * in the UHVW format. The check starts at the mEnd end and works backwards. This is included so the the user has the chance to
	 * influence the performance if they can determine which end is likely to fail quickest.
	 * 
	 * @param pDistance
	 *            the distance
	 * @return returns false the first time that a pixel fails the test. It will return true if all pixels are closer than mDistance
