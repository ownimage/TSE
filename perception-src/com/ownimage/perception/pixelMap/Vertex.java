...
public class Vertex implements IVertex {
	...
	private void calcTangent() {
		if (getStartSegment() == null && getEndSegment() == null) { return; }
	...
	public int compareTo(final IVertex pVertex) {
		if (pVertex == null) { throw new NullPointerException("pVertex must not be  null"); }

		if (samePosition(pVertex)) { return 0; }

		if (getX() > pVertex.getX() || getX() == pVertex.getX() && getY() > pVertex.getY()) { return 1; }
	...
	public IVertex deepCopy(final IVertex pOriginalPCStartVertex, final IVertex pCopyPCStartVertex) {
		...
		if (pOriginalPCStartVertex == this) { return pCopyPCStartVertex; }
	...
	public boolean equals(final Object pObject) {
		if (!(pObject instanceof Vertex)) { return false; }
	...
	public void setIndex(final int pIndex) {
		// if (mStartSegment != null && !(pIndex > mStartSegment.getStartIndex())) { throw new IllegalArgumentException("pIndex must
		// be greater than mStartSegment's starting index"); }

		if (mEndSegment != null && !(pIndex < mEndSegment.getEndIndex())) { throw new IllegalArgumentException("pIndex must be less than than mEndSegment's end index"); }

		if (isFixed()) { throw new IllegalStateException("Cannot set the index on a Vertex that isFixed()"); }

		if (mIndex == pIndex) { return; }
	...
	@Override
	public void setSmooth(final boolean pSmooth) {
		mSmooth = pSmooth;
	}
