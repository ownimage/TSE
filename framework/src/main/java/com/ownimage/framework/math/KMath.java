...
public class KMath {
     ...
     *
     * @param pValue the value
     * @param pRange the range value must be > 0.0d
     * @return the value
     */
    public static double mod(final double pValue, final double pRange) {
        if (pRange <= 0.0d) {
            throw new IllegalArgumentException("pRange must be > 0.0d, value supplied " + pRange);
        }

        if (0.0d <= pValue && pValue < pRange) {
            return pValue;
        }

        if (pValue < 0.0d) {
            return mod(pValue + pRange, pRange);
        }
