...
// this class taken from https://github.com/Rogach/javafx-group-layout/blob/master/GroupLayoutPane.java
...
public class GroupLayoutPane extends Pane {
	...
	public class SequentialGroup extends Group {
		...
		void recalculateSizes() {
			...
			this.minSize = springs.stream().mapToDouble(spring -> spring.getMinSize()).sum();
			this.prefSize = springs.stream().mapToDouble(spring -> spring.getPrefSize()).sum();
			this.maxSize = springs.stream().mapToDouble(spring -> spring.getMaxSize()).sum();
		}
		...
		void resizeRelocate(double location, double size) {
			...
			Collections.sort(sorted, (a, b) -> Double.compare(invPossibleChange.apply(a), invPossibleChange.apply(b)));
	...
	public class ParallelGroup extends Group {
		...
		void recalculateSizes() {
			...
			this.minSize = snapSize(springs.stream().mapToDouble(s -> s.getMinSize()).max().orElse(0));
			this.maxSize = snapSize(springs.stream().mapToDouble(s -> s.getMaxSize()).min().orElse(0));
			double prefSize = snapSize(springs.stream().mapToDouble(s -> s.getPrefSize()).max().orElse(0));
